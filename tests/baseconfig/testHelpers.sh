...
# Usage: check_relay_fees_with_retry <relay_container> <initial_balance> <balance_cmd> <retry_delay> <all_containers>
# Returns: 1 if balance increased, 0 if not
# Does NOT loop - waits once, processes queues, retries once
check_relay_fees_with_retry() {
    local relay="$1"
    local initial_balance="$2"
    local balance_cmd="$3"
    local retry_delay="${4:-15}"
    local all_containers="$5"

    # First check
    local new_balance=$(docker exec ${relay} sh -c "$balance_cmd" 2>/dev/null || echo "$initial_balance")
    local increased=$(awk "BEGIN {print ($new_balance > $initial_balance) ? 1 : 0}")

    # If not increased, retry after processing queues
    if [ "$increased" -eq 0 ]; then
        sleep "$retry_delay"
        # Process all queues for routing
        process_routing_queues "$all_containers"
        # Recheck
        new_balance=$(docker exec ${relay} sh -c "$balance_cmd" 2>/dev/null || echo "$initial_balance")
        increased=$(awk "BEGIN {print ($new_balance > $initial_balance) ? 1 : 0}")
    fi
...
# Check end-to-end delivery with retry after queue processing
# Usage: check_delivery_with_retry <container> <check_cmd> <initial_value> <retry_delay> <all_containers>
# Returns: final value (echoed), return code 0 if increased, 1 if not
check_delivery_with_retry() {
    local container="$1"
    local check_cmd="$2"
    ...
    # First check
    local final_value=$(docker exec ${container} sh -c "$check_cmd" 2>/dev/null || echo "$initial_value")
    local increased=$(awk "BEGIN {print ($final_value > $initial_value) ? 1 : 0}")
    ...
    if [ "$increased" -eq 0 ]; then
        ...
        process_routing_queues "$all_containers"
        # Recheck
        final_value=$(docker exec ${container} sh -c "$check_cmd" 2>/dev/null || echo "$initial_value")
        increased=$(awk "BEGIN {print ($final_value > $initial_value) ? 1 : 0}")
    fi
