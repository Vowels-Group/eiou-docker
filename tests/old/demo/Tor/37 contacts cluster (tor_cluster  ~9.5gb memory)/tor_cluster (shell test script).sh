...
echo -e "\nCreating containers..."
for container in "${containers[@]}"; do
    docker run -d --network=eiou-network --name $container -v "${container}-mysql-data:/var/lib/mysql" -v "${container}-files:/etc/eiou/" -v "${container}-index:/var/www/html" -v "${container}-eiou:/usr/local/bin/" -v "${container}-backups:/var/lib/eiou/backups" eiou/eiou
done
...
# Send money
echo -e "\nSending money..."
docker exec torA eiou send ${containerAddresses[torA422]} 100 USD
docker exec torA eiou send ${containerAddresses[torA312]} 100 USD
docker exec torA eiou send ${containerAddresses[torA2]} 100 USD
docker exec torA eiou send ${containerAddresses[torA21]} 100 USD
docker exec torA eiou send ${containerAddresses[torA412]} 100 USD
docker exec torA eiou send ${containerAddresses[torA121]} 100 USD
...
echo -e "\nViewing balances of torA422"
docker exec torA422 eiou viewbalances

echo -e "\nViewing balances of torA312"
docker exec torA312 eiou viewbalances
...
echo -e "\nViewing history of torA422"
docker exec torA422 eiou history

echo -e "\nViewing history of torA312"
docker exec torA312 eiou history
