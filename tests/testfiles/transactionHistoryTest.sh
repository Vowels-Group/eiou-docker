...
for container in "${containers[@]}"; do
    ...
    fieldCheck=$(docker exec ${container} php -r "
        ...
        \$stmt = \$db->prepare('
            SELECT id, type, amount, currency, sender_address, recipient_address,
                   timestamp, status, fee, message_hash
            FROM transactions
            LIMIT 1
        ');
        \$stmt->execute();
        ...
        if (\$transaction) {
            \$requiredFields = ['id', 'type', 'amount', 'timestamp'];
            \$missingFields = [];
...
if [[ "$testPair" ]]; then
    ...
    senderBefore=$(docker exec ${sender} php -r "
        require_once('./etc/eiou/src/services/ServiceContainer.php');
        \$db = ServiceContainer::getInstance()->getDatabase();
        \$stmt = \$db->prepare('SELECT COUNT(*) FROM transactions WHERE type = \"send\"');
        \$stmt->execute();
...
for container in "${containers[@]:0:2}"; do  # Test first 2 containers
    ...
    queryResult=$(docker exec ${container} php -r "
        ...
        ');
        \$stmt->execute();
        \$sends = \$stmt->fetch(PDO::FETCH_ASSOC);
        ...
        ');
        \$stmt->execute();
        \$receives = \$stmt->fetch(PDO::FETCH_ASSOC);
