...
class PluginPoolService
{
    ...
    /**
     * Per-plugin credentials-file directory. Each plugin's credentials
     * file at `/etc/eiou/credentials/plugin-<id>.json` is added to the
     * pool's open_basedir as an exact file path so the plugin can
     * authenticate as its own MySQL user without crossing the gateway
     * for every query. The group on the file (`eiou-pc-<hash>`) plus
     * the exact-path entry below mean a plugin can ONLY read its own
     * file: a sibling plugin's `plugin-<other>.json` is denied at the
     * group layer even though the directory itself is in basedir.
     */
    ...
    public function renderPoolConfig(string $pluginId, string $systemUser): string
    {
        ...
        // Credentials file is added as an EXACT file path (no trailing
        // slash) so the pool can open its own `plugin-<id>.json` but
        // nothing else under /etc/eiou/credentials/. Cross-plugin reads
        // are still blocked at the group layer (the pool user is only
        // in its own `eiou-pc-<hash>` group), but tightening
        // open_basedir to a single file is defence in depth.
        $credentialsFile = self::CREDENTIALS_DIR . '/plugin-' . $pluginId . '.json';
...
EOT;
    ...
    {
        $path = \Eiou\Services\Plugins\PluginGatewayTokenService::DEFAULT_TOKENS_PATH;
