 ...
 * Trust model:
 *
 *   - The plugin's own FPM pool (`eiou-p-<hash>`) CAN read its own
 *     credentials file. The supervisor adds the pool user to the
 *     per-plugin `eiou-pc-<same-hash>` group during apply-credentials,
 *     and `PluginPoolService::renderPoolConfig` extends the pool's
 *     `open_basedir` to admit the file's exact path (no other file
 *     under `/etc/eiou/credentials/`). The plugin uses this to
 *     `new PDO(...)` directly against its own MySQL user — no
 *     gateway round-trip for owned-table queries.
 *
 *   - Cross-plugin reads are denied at the group layer: plugin A's
 *     pool user is only in `eiou-pc-<hash(A)>`, never in any other
 *     plugin's group. The exact-file `open_basedir` entry is
 *     defence in depth — even if group membership were misconfigured,
 *     a sibling plugin's `plugin-<other>.json` is outside basedir.
 *
 *   - The wallet pool (www-data) is NOT in any of these groups. It
 *     doesn't need on-disk access: it has the master key and decrypts
 *     credentials from the `plugin_credentials` table in memory
 *     whenever it needs to act on them.
 *
 *   - Operator-deployed sibling containers mount the file by absolute
 *     path and arrange sibling-side UID to be in the host's
 *     `eiou-pc-<hash>` group (one plugin id per sibling, one group
 *     per plugin). The host's responsibility ends at "produce a 0640
 *     root:eiou-pc-<hash> file and add the pool user"; everything
 *     beyond is operator policy.
 *
