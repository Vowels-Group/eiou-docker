...
# Plugin sandboxing: eiou-locations.conf includes a per-plugin routing
# snippet that is regenerated by PluginNginxConfigService when a
# sandboxed plugin enables or disables. Bake an empty (header-only) file
# at image build time so a fresh node — with no sandboxed plugins — still
# satisfies the include and `nginx -t` succeeds at first boot.
# See docs/PLUGINS.md (Sandboxing) for the broader trust model.
RUN mkdir -p /etc/nginx/snippets && \
    printf '# Auto-generated by PluginNginxConfigService — initially empty.\n# Populated when a sandboxed plugin enables.\n' \
        > /etc/nginx/snippets/eiou-plugins.conf

# Same shape for the http{}-scope limit_req_zone declarations file
# that backs public-route rate limiting. Empty at image build; the
# routing poller rewrites it on every plugin enable/disable.
RUN mkdir -p /etc/nginx/conf.d && \
    printf '# Auto-generated by PluginNginxConfigService — initially empty.\n# Populated when a sandboxed plugin with public_routes enables.\n' \
        > /etc/nginx/conf.d/eiou-plugin-zones.conf

# Plugin sandboxing (Phase 2): per-plugin writable scratch dir root.
# Created here so the supervisor doesn't have to mkdir at runtime; each
# plugin's subdir (chowned to its eiou-p-<hash> user) is materialised
# at enable time.
RUN mkdir -p /var/lib/eiou/plugin-scratch && \
    chmod 755 /var/lib/eiou/plugin-scratch
...
# own trusted keys to /etc/eiou/plugins/trusted-keys/ on the plugins volume.
# See docs/PLUGINS.md → Plugin Signatures.
COPY files/trusted-plugin-keys/ /app/eiou/plugins/trusted-keys/
