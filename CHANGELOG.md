...
- **`ApiKeysController` GUI controller + AJAX endpoints**: new `Eiou\Gui\Controllers\ApiKeysController` wires through `Functions.php` alongside the other AJAX controllers. Actions: `apiKeysStatus`, `apiKeysVerify`, `apiKeysClearAccess`, `apiKeysList`, `apiKeysCreate`, `apiKeysToggle`, `apiKeysDelete`. CSRF is validated (non-rotating) on every call; payload validation reuses `ApiKeyService::validatePermissions()` + `validateRateLimit()` so CLI and GUI stay in lockstep on what's accepted. `key_id` values submitted to mutating actions are shape-checked (`eiou_` + 24 hex chars) before hitting the DB. GUI-triggered creations, toggles, and deletions are written to the unified logger — which masks API-key/secret patterns via `SecureLogger`, so the audit trail never leaks a shown-once secret
