...
- **QR code support**: addresses in Wallet Information now have a QR code button that toggles an SVG QR code display. The Add Contact modal has a camera scan button (📷) next to the address input — opens the device camera to scan a QR code and auto-fills the address. Uses bundled `qrcode-generator` (8KB, MIT) for display and `html5-qrcode` (375KB, Apache-2.0) for scanning. No CDN dependency — works on TOR-only nodes. Scanner supports rear camera (mobile) and webcams (desktop)
