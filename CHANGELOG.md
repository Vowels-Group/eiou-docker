...
- Add AAD context to AES-256-GCM encryption with backward-compatible versioned format (L-28) — `KeyEncryption::encrypt()` accepts optional `$context` param used as Additional Authenticated Data; output includes `version` and `aad` fields; `decrypt()` auto-detects v1 (empty AAD) vs v2 format; callers pass context strings (`private_key`, `auth_code`, `mnemonic`, `api_secret`, `backup`)
- Strengthen P2P message hash with sender public key binding (M-18) — hash formula changed from `hash(address + salt + time)` to `hash(senderPublicKey + address + salt + time)`; all matching functions (`matchYourselfP2P`, `matchContact`, `checkMessageValidity`) check both new and legacy formats for backward compatibility
