...
- **GUI login page 500 error after GUI asset audit**: the prior commit refactored `authenticationForm.html` and `banner.html` to call template helpers (`cspNonce()`, `appVersion()`, `getAlphaWarning()`, `autoLinkUrls()`, `getBanners()`, `appVersionDisplay()`) that live in `src/gui/functions/Functions.php` — but that file is only included AFTER authentication at `index.html` line 138, so on the unauthenticated login page every helper call was a fatal undefined-function error. Extracted the pure template helpers to a new `src/gui/functions/TemplateHelpers.php` and included them from `index.html` BEFORE the auth check, so both the login page and the wallet page see the same helpers. `Functions.php` still pulls them in via an idempotent `require_once` so the authenticated path is unchanged
