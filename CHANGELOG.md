...
### Changed
- **Recent Transactions list → tabular layout**: the Activity → Recent Transactions section is now a true table that mirrors the Your Contacts table chrome (`contacts-table` class with a `tx-table` modifier). Columns, in order: **Counterparty** (avatar + name), **Amount** (sortable, ±value with the user's amount color scheme), **Description** (CSS-truncated with full text on hover; row click still opens the existing transaction modal showing full details), **Direction** (Sent / Received pill), **Type** (Direct / P2P / Contact-request badge), **Final Recipient** (avatar + name when known, truncated address otherwise — only populated for outgoing P2P I originated, via `end_recipient_address`), **Status** (existing status badge with status icon), **Date** (sortable, formatted timestamp). New filter row above the table mirrors the Contacts filter pattern with three dropdowns: Any direction / Any type / Any status. Sortable columns (Amount, Date) reuse the same asc → desc → off cycle as the Contacts table via a new `sortTransactions()` / `updateTransactionsSortIndicators()` pair in `script.js`. `filterTransactions()` extended to AND-combine the search input with the three dropdown filters. DLQ indicator collapses from a full badge to a small `.dlq-tx-icon` next to the Status badge. Mobile (≤600px) collapses to **Counterparty | Amount | Status icon** — the +/− on Amount carries direction, and the Status icon (e.g. `fa-check-double` for completed) confirms the tx actually went through so users don't have to assume. `markTruncatedContactNumberCells()` is now scoped to `.contacts-table:not(.tx-table)` so its injected `showInfoModal` action doesn't steal clicks from tx rows
