...
- Best-fee mode broadcast path now counts `already_relayed` contacts in `contacts_sent_count` alongside `inserted`: the `already_relayed` contact records the broadcaster as an additional sender (`p2p_senders`) and will forward its best RP2P back, so waiting for its response improves route quality. Per-hop expiration prevents the circular deadlock that originally motivated excluding `already_relayed` — if the contact never responds, cleanup selects at hop-wait expiration
