...
### Fixed
- **False-positive chain gap when cancelled transaction is in the chain**: `verifyChainIntegrity()` excluded cancelled/rejected transactions from the chain walk, but `previous_txid` can legitimately point to a cancelled transaction (since commit `04239bc6` preserved chain integrity by inclusion). This caused a permanent "Chain Gap" status on contacts when a transaction was cancelled after delivery failure, even though sync correctly syncs cancelled transactions. Removed the `status NOT IN ('cancelled', 'rejected')` filter so the integrity check includes all transactions, matching the rest of the chain logic
