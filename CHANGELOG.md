...
### Changed
- **`runuser` absolute-path discipline extended to PHP**: new `Constants::BIN_RUNUSER = '/usr/sbin/runuser'`. All three PHP sites that exec `analytics-cron.php` on the analytics opt-in toggle (`SettingsController::triggerInitialAnalytics`, `CliSettingsService::changeSetting`, `ApiController` settings PUT) now build their command from `Constants::BIN_RUNUSER` instead of a bare `runuser` literal. v0.1.11 fixed the three `startup.sh` cron definitions that hit the same "cron's minimal PATH doesn't include `/usr/sbin`" failure mode, but the PHP opt-in-trigger paths still used bare `runuser` — a latent silent-failure hazard for the initial `node_setup` event (the PHP `@exec()` shell's PATH isn't guaranteed to include `/usr/sbin` either, and the command is backgrounded with `&` so any failure disappears into the log). Having a single constant keeps future image-migration concerns (e.g. Alpine ships `runuser` elsewhere or not at all) a one-line change
