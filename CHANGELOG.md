...
- **Failed Messages mobile layout**: bulk actions (Retry All, Abandon All, Refresh) and per-row actions (Retry, Abandon) collapse into compact dropdown menus on mobile. Section padding/margin matched to other sections. Truncated Failure Reason and Recipient text is now tappable to show full content in a modal (same pattern as wallet info icons)
