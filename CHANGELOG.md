...
- **Unit tests** for `PaymentRequestRepository` (16 tests) and `PaymentRequestService` (28 tests) — full coverage of create, approve, decline, cancel, incoming message handling, response handling, and description-prefix formatting logic
...
- **Payment requests landing in DLQ despite successful delivery**: the `type=message` handler in `index.html` called `handleMessageRequest()` (which is `void`) but never echoed an HTTP response — the sender's `MessageDeliveryService` received an empty body, treated it as "no response received", retried until exhaustion, and moved the message to the DLQ. Alice had the payment request all along. Fixed by echoing `{"success":true,"status":"received"}` after a successful `handleMessageRequest()` call, matching the pattern used by every other message type handler in the same file. This also fixes contact acceptance messages (type=message, typeMessage=contact) hitting the same DLQ path
