...
### Fixed
- Fix duplicate retry workers for the same message — `attemptDeliveryWithRetries` now sets `next_retry_at` to the actual sleep delay (not `0`) before sleeping, so `processRetryQueue` sees the message as locked and will not claim it mid-sleep and create a parallel retry chain
